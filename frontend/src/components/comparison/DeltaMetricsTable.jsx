  ...
  if (!deltaMetrics?.metrics?.length) {
    return (
      <div className = "flex items-center justify-center py-8">
        <p className = "text-xs text-slate-600">No metrics available yet</p>
      </div>
    )
  }
      ...
      <table className = "w-full text-xs">
        ...
        <tbody>
          {deltaMetrics.metrics.map((metric) => (
            <tr key = {metric.key} className = "border-b border-white/[0.04] hover:bg-slate-800/30">
              <td className = "py-2 px-3 text-slate-400">{metric.label}</td>
              {slots.map((slot) => {
                ...
                return (
                  <td key = {slot.id} className = "text-right py-2 px-3">
                    <span className = {cn(
                      ...
                      isBest ? 'text-state-success font-medium' : 'text-slate-300'
                    )}>
                      {value ?? '—'}
                    </span>
                    {delta != null && delta > 0 && (
