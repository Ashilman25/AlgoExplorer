...
export default function ComingSoon({ icon: Icon, title, description, features = [] }) {
  return (
    <div className = "flex flex-col items-center justify-center min-h-[52vh] py-10 text-center animate-enter stagger-1">
      {Icon && (
        <div className = "p-4 rounded-2xl bg-slate-800/70 border border-white/[0.07] mb-5">
          <Icon size = {28} strokeWidth = {1} className="text-slate-600" />
        </div>
      )}

      <h2 className = "text-base font-semibold text-slate-300 mb-2 max-w-sm">
        {title}
      </h2>
