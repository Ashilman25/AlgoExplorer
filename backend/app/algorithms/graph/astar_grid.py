...
@register("graph", "astar_grid")
class AStarGridAlgorithm(BaseAlgorithm):
    @property
    ...
    def run(self, algo_input: AlgorithmInput) -> AlgorithmOutput:
        ...
        if config.allow_diagonal:
            ...
            def heuristic(r: int, c: int) -> float:
                return float(abs(r - tr) + abs(c - tc))

        # ── benchmark fast path ─────────────────────────────
        if algo_input.execution_mode == "benchmark":
            g_scores: dict[tuple[int, int], float] = {config.start: 0}
            h_scores: dict[tuple[int, int], float] = {config.start: heuristic(sr, sc)}
            visited: set[tuple[int, int]] = set()
            parent: dict[tuple[int, int], tuple[int, int]] = {}
            counter = 0
            f_start = g_scores[config.start] + h_scores[config.start]
            heap: list[tuple[float, int, tuple[int, int]]] = [(f_start, counter, config.start)]
            metrics = {
                "cells_explored": 0,
                "frontier_max_size": 1,
                "path_length": 0,
            ...
            while heap:
                ...
                for (nr, nc), cost in adj[current]:
                    ...
                    if new_g < g_scores.get((nr, nc), math.inf):
                        g_scores[(nr, nc)] = new_g
                        if (nr, nc) not in h_scores:
                            h_scores[(nr, nc)] = heuristic(nr, nc)
                        f = new_g + h_scores[(nr, nc)]
                        parent[(nr, nc)] = current
                        counter += 1
                        heapq.heappush(heap, (f, counter, (nr, nc)))
        ...
        def dist_display() -> dict[str, float | str]:
            ...
            for (r, c), g in g_scores.items():
                result[coord_key(r, c)] = round(g, 2) if g != math.inf else "inf"
            return result

        def heuristic_display() -> dict[str, dict[str, float | str]]:
            result: dict[str, dict[str, float | str]] = {}
            for cell, g in g_scores.items():
                if g == math.inf:
                    continue
                r, c = cell
                h = h_scores.get(cell, heuristic(r, c))
                f = g + h
                result[coord_key(r, c)] = {
                    "g": round(g, 2),
                    "h": round(h, 2),
                    "f": round(f, 2),
                }
        ...
        steps.append(make_grid_step(
            ...
            explanation = eb.build(
                title = f"Initialize A* from ({sr},{sc})",
                body = f"Heuristic: {heuristic_name} distance. h({sr},{sc}) = {round(h_scores[config.start], 2)}, f({sr},{sc}) = {round(f_start, 2)}. Target is ({tr},{tc}).",
                data_snapshot = {
        ...
        while heap:
            ...
            steps.append(make_grid_step(
                ...
                metrics_snapshot = dict(metrics),
                explanation = eb.build(
                    title = f"Pop ({cr},{cc}) with f={round(current_f, 2)} (g={round(current_g, 2)}, h={round(current_h, 2)})",
            ...
            if current == config.end:
                ...
                steps.append(make_grid_step(
                    ...
                    explanation = eb.build(
                        ...
                        body = f"Shortest distance: {round(current_g, 2)}. Path length: {len(final_path)} cells. A* guarantees optimal path with an admissible heuristic.",
                        data_snapshot = {
                            "path": final_path,
                            "distances": dist_display(),
                            "heuristic_values": heuristic_display(),
            ...
            for (nr, nc), cost in adj[current]:
                if (nr, nc) in visited:
                    continue  # skip silently for grids
                ...
                old_g = g_scores.get((nr, nc), math.inf)
                ...
                if new_g < old_g:
                    ...
                    steps.append(make_grid_step(
                        ...
                        explanation = eb.build(
                            ...
                            body = f"g: {old_label} -> {round(new_g, 2)}, f={round(f, 2)} (h={round(h, 2)}). Pushed to heap.",
                            data_snapshot = {
                                "distances": dist_display(),
                                "heuristic_values": heuristic_display(),
                                "edge_cost": cost,
                    ...
                    steps.append(make_grid_step(
                        ...
                        explanation = eb.build(
                            ...
                            body = f"Current g={round(old_g, 2)} <= {round(new_g, 2)}.",
                            data_snapshot = {
                                "distances": dist_display(),
                                "heuristic_values": heuristic_display(),
                            },
        ...
        path_cost = round(g_scores.get(config.end, math.inf), 2) if path_found else None
